...
async def handle_call_tool(name: str, arguments: dict):
    ...
    if name == "get_transcript":
        ...
        return await list_transcripts(arguments)
    else:
        raise ValueError(f"Unknown tool: {name}")


async def get_transcript(arguments: Dict[str, Any]):
    ...
    try:
        ...
        try:
            ...
            return [
                TextContent(
                    type="text",
                    text=f"Error: Transcripts are disabled for video {video_id}",
                )
            ]
        except VideoUnavailable:
            return [
                TextContent(type="text", text=f"Error: Video {video_id} is unavailable")
            ]
        except NoTranscriptFound as e:
            return [
                TextContent(
                    type="text",
                    text=f"Error: No transcript found for video {video_id} in language '{lang}' or English fallback: {str(e)}",
                )
            ]
...
async def list_transcripts(arguments: Dict[str, Any]):
            ...
            for transcript in transcript_list:
                info = {
                    ...
                    "translation_languages": [
                        lang.language_code
                        for lang in transcript.translation_languages[:5]
                    ],  # Get language codes and limit
                }
